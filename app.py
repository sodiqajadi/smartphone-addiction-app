...
<style>
    ...
    .badge-low    { background:#E1F5EE; color:#0F6E56; border-radius:20px; padding:6px 18px; font-weight:700; font-size:16px; }
    .badge-medium { background:#FAEEDA; color:#854F0B; border-radius:20px; padding:6px 18px; font-weight:700; font-size:16px; }
    .badge-high   { background:#FAECE7; color:#993C1D; border-radius:20px; padding:6px 18px; font-weight:700; font-size:16px; }
...
def build_raw(screen, social, gaming, work_study, sleep,
              notifs, app_opens, weekend, stress, academic, gender, age):
    return dict(
        age=age,
...
def make_gauge(prob: float) -> go.Figure:
    ...
    fig   = go.Figure(go.Indicator(
        ...
        title={"text":f"<b>{tier} Risk</b>",
               "font":{"size":20,"color":C_NAVY,"family":"Arial"}},
    ))
    fig.update_layout(
...
def make_waterfall(X: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title=dict(text="<b>What drove this prediction?</b>",
...
def make_comparison(raw: dict) -> go.Figure:
    ...
    user_v = [
        ...
        min(0.45*(dst/12)+0.35*(raw["weekend_screen_time"]/15)
            +0.20*(raw["social_media_hours"]/6), 1),
    ]
    feats  = ["avg_screen_time","avg_social_media","avg_sleep",
              "avg_weekend_screen","avg_pressure_score"]
...
def make_global_shap() -> go.Figure:
    ...
    colors = [C_CORAL if v>q70 else C_AMBER if v>q40 else C_GRAY
              for v in top10["mean_shap"]]
    fig = go.Figure(go.Bar(
        x=top10["mean_shap"],
    ...
    for fi in np.argsort(sv)[::-1][:8]:
        ...
        if feat=="daily_screen_time_hours" and val>6:
            ...
            recs.append(("🎯","Replace passive use with purpose",
                f"**{val:.1f} hrs/day** non-productive. "
...
with st.sidebar:
    ...
    <div style='text-align:center; padding:10px 0 20px'>
        ...
        <div style='font-size:1.1rem; font-weight:700'>Usage Profile</div>
    ...
    gender = st.radio("Gender", ["Female","Male","Other"], horizontal=True, index=1)
    ...
    sleep    = st.slider("Sleep hours",   4.5, 9.0, 7.0, 0.5)
    stress   = st.radio("Stress level",  ["Low","Medium","High"], horizontal=True, index=1)
    academic = st.radio("Affects work/study?", ["No","Yes"], horizontal=True)
...
with tab1:
    if not predict_btn:
        ...
        with st.expander("📊 View global feature importance"):
            st.plotly_chart(make_global_shap(),
                            use_container_width=True, key="global_shap_1")
    else:
        raw = build_raw(daily_screen, social_media, gaming, work_study, sleep,
                        notifs, app_opens, weekend_screen, stress, academic, gender, age)
        X    = engineer(raw)
        ...
        with col_gauge:
            st.plotly_chart(make_gauge(prob),
        ...
        with col_right:
            ...
            if tier == "High":
                st.error("🚨 Your usage matches **high-risk users**. "
                         "Review the recommendations below.")
            elif tier == "Medium":
                st.warning("⚠️ Your usage is **borderline**. Small changes could "
                           "meaningfully lower your risk.")
            else:
                st.success("✅ Your usage is associated with **low addiction risk**. "
                           "Keep it up!")
        ...
        with col_shap:
            st.plotly_chart(make_waterfall(X),
                            use_container_width=True, key="waterfall")
        with col_comp:
            st.plotly_chart(make_comparison(raw),
                            use_container_width=True, key="comparison")
        ...
        with st.expander("🔧 Full feature values"):
            ...
            st.dataframe(feat_df.style.format({"Value":"{:.3f}","SHAP":"{:+.4f}"}),
                         use_container_width=True)

# ════════════════════════════════════════════════════════
# TAB 2 — WHAT-IF  (always computes — no dependency on predict_btn)
# ════════════════════════════════════════════════════════
with tab2:
    st.markdown("""
    ### 🔬 What if I changed my habits?
    Adjust these sliders to instantly see how changing a behaviour
    ...
    # Always compute both probabilities from sidebar inputs — no predict_btn needed
    raw_cur = build_raw(daily_screen, social_media, gaming, work_study, sleep,
                        notifs, app_opens, weekend_screen, stress, academic, gender, age)
    raw_sim = build_raw(sim_screen, sim_social, gaming, work_study, sim_sleep,
                        notifs, app_opens, sim_wknd, stress, academic, gender, age)
    ...
    if delta < -0.5:
        st.success(f"**{arrow} {delta:+.1f} percentage points** — "
                   f"simulated changes reduce your risk "
                   f"({prob_cur*100:.1f}% → {prob_sim*100:.1f}%, {tier_sim} risk)")
    elif delta > 0.5:
        st.error(f"**{arrow} {delta:+.1f} percentage points** — "
                 f"simulated changes increase your risk "
                 f"({prob_cur*100:.1f}% → {prob_sim*100:.1f}%, {tier_sim} risk)")
    else:
        st.info(f"**● Minimal change** — "
                f"risk stays at ~{prob_sim*100:.1f}% ({tier_sim} risk)")
...
with tab3:
    ...
    with col_info:
        ...
        > Social media hours and daily screen time are the two strongest
        > predictors. Age, gender, stress, and gaming add virtually no signal.
    ...
    with col_shap:
        ...
        st.plotly_chart(make_global_shap(),
                        use_container_width=True, key="global_shap_2")
        st.caption("Mean |SHAP| — each feature's average contribution "
                   "across all 7,500 users.")
...
with tab4:
    ...
    for col, emoji, level, desc, avg in [
        (c1,"✅","None","Usage is healthy and balanced.","Avg screen: ~5.2 hrs/day"),
        (c2,"🔵","Mild","Early signs — elevated but manageable.","Avg screen: ~6–7 hrs/day"),
        (c3,"🟡","Moderate","Clear dependency — interferes with daily life.","Avg screen: ~8 hrs/day"),
        (c4,"🔴","Severe","Strong addiction — significant impairment.","Avg screen: ~9–12 hrs/day"),
    ]:
        with col:
            st.markdown(f"""
    ...
    st.warning("""
    **Key finding:** Even an 80% reduction in any single feature is **not enough**
    to de-risk confirmed high-risk users. Effective intervention requires
    simultaneous changes across multiple behaviours.
    """)
...
<div style='text-align:center; color:#aaa; font-size:0.8rem; padding-bottom:16px'>
    Built with Streamlit · Random Forest (400 trees) · ROC-AUC 0.989<br>
